<?php
require_once("lib/rest/RestClientInfo.php");
...
class RestClient
{
    ...
    function __construct()
    {
        ...
        if (empty($this->dbm)) {
            Log::fatal('RestClient: Unable to connect to Database ...');
    ...
    public function trackClientRequest(ClientInfo &$clientInfo) {
        ...
        try {
            ...
            if (!$updated && $errno == DBManager::MYSQL_CODE_RECORD_NOT_FOUND ) {
                ...
                for ($i = 0; $i < self::MAX_RETRIES && ($errno != 0); $i++) {
                    ...
                    if ($errno != DBManager::MYSQL_CODE_DUPLICATE_KEY && $this->dbm->affectedRows($stmt) != 1) {
                        throw new Exception('RestClient: trackClientRequest FAILED - ERRNO=' . $errno);
                    }
                    $this->dbm->closeStatement($stmt);
                }
            }
            ...
            if ($errno) {
                $logMessage = 'RestClient: Failed (ERRNO: ' . $errno . ') trying to Insert new Client:'
